...
![Milestone](https://img.shields.io/badge/里程碑-M1%20已完成-2ea44f)
...
> 模型、搜索、研究、图片、浏览器插件业务和发布包仍未实现；应用不会自动操作小红书。

## 当前开发状态

**M0（Issue 001—005）与 M1（Issue 006—011）均已完成；M1 本地基础已完成验收。**

| 里程碑 | 范围                                                       |   状态    |
| ------ | ---------------------------------------------------------- | :-------: |
| M0     | TypeScript 单仓库、领域规则、硬约束、架构测试、Windows CI  |  ✅ 完成  |
| M1     | Electron、SQLite、任务队列、文件仓库、设置与凭据、本地 API |  ✅ 完成  |
| M2     | 供应商接口、能力探测、搜索、书库、研究、浏览器插件         | ⏳ 未开始 |
| M3—M6  | 内容生产、视觉、审批、导出、数据闭环、Windows 发布         | ⏳ 未开始 |

截至目前已完成 Issue 001—011 的全部本地基础事项。下一项是
**Issue 012：供应商无关的模型接口**，但仓库中尚无真实 provider 客户端，也不会在安装、
测试或启动时访问模型服务。
...
- source 与 packaged Electron smoke、依赖审计、500+ 项自动化测试和 Windows CI。

### 尚未具备

- 真实模型、搜索、联网研究、图片生成、OCR 或任何付费 API 调用。
...
flowchart LR
    ...
    Queue -. "Issue 012 才实现调用" .-> Providers["供应商接口"]
```
...
- `packages/core`、`db`、`storage`、`workflows`、`settings`、`local-api` 保持 Electron 无关。
- `apps/clipper` 和 `packages/providers` 目前仅保留后续里程碑所需的空包边界。
...
packages/
  ...
  providers/     Issue 012 预留包边界，尚无外部客户端
docs/
  adr/           架构决策记录
  contracts/     稳定协议合同
