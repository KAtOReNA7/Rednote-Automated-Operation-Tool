...
> R10A 与 R10B1A—R10B1C 已进入 `main`，受控备份核心已经实现；完整端到端恢复、诊断、安装升级和
> 候选验证尚未开始实施。
...
- **本地数据**：计划、内容包、互动、指标和状态保存在本机 SQLite 与受控文件目录中；受控备份核心已实现，完整端到端恢复仍未实施。
...
| **R10**    | **安装、升级、备份恢复、诊断和 Windows 端到端验收**   | **R10A、R10B1A—R10B1C 已合并；后续待独立授权** |

N1—N7 已按[最终视觉统一政策](./docs/product/v2-final-visual-convergence-policy.md)完成并获用户验收。
R08、R09 已合并；R09 的只读 Catalog 已可用。R10A 与 R10B1A—R10B1C 已合并，受控备份核心已实现；
完整端到端恢复、R10C 脱敏诊断、R10D Windows 安装/升级/卸载和 R10E Release Candidate 尚未开始，均等待独立决策与授权。
...
<p align="center">
  ...
  <br />
  V2-R01—R09 已验收并合并；R10A 与 R10B1A—R10B1C 已合并，完整恢复与后续发布准备仍待独立授权；最终发布和互动发送始终由用户手动完成。
</p>
