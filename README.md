...
> 预览、目录选择和明确确认后才写入的脱敏本地诊断包；R10D 正在为未签名内部 beta 实现每用户离线安装、手动升级与保留数据卸载，R10E 候选验证尚未开始。
...
| **R10**    | **安装、升级、备份恢复、诊断和 Windows 端到端验收**   | **R10B 已合并；R10C 提供受控本地诊断；R10D 安装器在当前代码版本中实现，R10E 待独立授权** |
...
R10C 提供受控本地脱敏诊断导出；R10D Windows 每用户离线安装、手动升级与保留数据卸载在当前代码版本中实现，仍须通过精确 Windows CI；R10E Release Candidate 等待独立授权。
...
<p align="center">
  ...
  <br />
  V2-R01—R09 已验收并合并；R10B 的完整恢复已通过 PR #26 合并，R10C 提供受控本地诊断，R10D—R10E 与后续发布准备仍待独立授权；最终发布和互动发送始终由用户手动完成。
</p>
