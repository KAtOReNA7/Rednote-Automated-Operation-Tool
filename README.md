...
> Provider 合同与用户显式能力探测已经建立，但内容工作流中的真实模型、搜索、研究、
> 图片业务、浏览器插件业务和发布包仍未接通；应用不会自动调用模型或操作小红书。
...
- 内容工作流中的真实 provider wiring、搜索、联网研究、正式图片保存、OCR 或自动付费调用。
...
- `packages/core`、`db`、`storage`、`workflows`、`settings`、`local-api`、`providers` 保持
  Electron 无关。
- `apps/clipper` 仍是 Issue 017 的空包；renderer、preload 与 local API 不直接导入
  `packages/providers`，应用启动、保存设置、迁移、定时器和队列不会自动探测。
...
packages/
  ...
  providers/     模型接口、能力探测、配置、usage、错误、transport、codecs 与 Mock
docs/
  adr/           架构决策记录
  contracts/     稳定协议合同
