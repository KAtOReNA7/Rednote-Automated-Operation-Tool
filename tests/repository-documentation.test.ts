...
describe('repository-facing documentation', () => {
  it('reports completed M2 and Issue 022 while keeping Issue 023 bounded', () => {
    expect(readme).toContain('M1（Issue 006—011）');
    expect(readme).toContain('M2（Issue 012—021）均已完成验收');
    expect(readme).toContain('M3 Issue 023（实验管理，仅规划，尚未授权或开始）');
  ...
  it('gives future agents the frozen safety, migration, validation, and Git boundaries', () => {
    for (const required of [
      ...
      '未经用户明确授权，不得 push',
      'M2（Issue 012—021）与 M3 Issue 022',
      '下一项规划是 M3 Issue 023',
      'migration v1—v15 已发布',
    ]) {
