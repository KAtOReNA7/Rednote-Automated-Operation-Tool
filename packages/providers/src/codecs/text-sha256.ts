...
/** A small synchronous SHA-256 used by codecs in both Node and browser bundles. */
export function textSha256(value: string): string {
  ...
  for (let offset = 0; offset < bytes.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const first = (h + big1 + choose + (K[index] ?? 0) + (words[index] ?? 0)) >>> 0;
    ...
    for (const [index, valuePart] of [a, b, c, d, e, f, g, h].entries())
      hash[index] = ((hash[index] ?? 0) + valuePart) >>> 0;
  }
