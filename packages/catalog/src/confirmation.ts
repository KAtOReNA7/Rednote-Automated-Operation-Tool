...
import { catalogSemanticHash } from './identity.js';
...
export interface CatalogConfirmationPreview<T> {
...
export class CatalogConfirmationBroker<T> {
  readonly #clock: () => Date;
  ...
  public constructor(clock: () => Date = () => new Date(), lifetimeMs = 5 * 60_000) {
    if (!Number.isSafeInteger(lifetimeMs) || lifetimeMs < 1_000 || lifetimeMs > 15 * 60_000) {
      throw new TypeError('confirmation lifetime is invalid');
    }
    this.#clock = clock;
    this.#lifetimeMs = lifetimeMs;
  }

  public issue(payload: T, senderId: number, windowId: number): CatalogConfirmationPreview<T> {
    this.prune();
    const token = randomBytes(32).toString('base64url');
    const previewHash = catalogSemanticHash(payload);
