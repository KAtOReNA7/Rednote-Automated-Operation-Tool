...
import { TOPIC_LIMITS } from './constants.js';
import { TopicError } from './errors.js';
import { topicSemanticHash } from './identity.js';

interface PendingTopicConfirmation<T> {
...
export interface TopicConfirmationPreview<T> {
...
export class TopicConfirmationBroker<T> {
  readonly #clock: () => Date;
  readonly #lifetimeMs: number;
  readonly #pending = new Map<string, PendingTopicConfirmation<T>>();

  public constructor(
    clock: () => Date = () => new Date(),
    lifetimeMs = TOPIC_LIMITS.confirmationTtlMs,
  ) {
    if (!Number.isSafeInteger(lifetimeMs) || lifetimeMs < 1_000 || lifetimeMs > 15 * 60_000) {
      throw new TypeError('Topic confirmation lifetime is invalid.');
    }
    this.#clock = clock;
    this.#lifetimeMs = lifetimeMs;
  }

  public issue(payload: T, senderId: number, windowId: number): TopicConfirmationPreview<T> {
    this.prune();
    const token = randomBytes(32).toString('base64url');
    const previewHash = topicSemanticHash(payload);
