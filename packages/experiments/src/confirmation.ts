...
import { EXPERIMENT_LIMITS } from './constants.js';
import { ExperimentError } from './errors.js';
import { experimentSemanticHash } from './identity.js';

interface PendingExperimentConfirmation<T> {
...
export interface ExperimentConfirmationPreview<T> {
...
export class ExperimentConfirmationBroker<T> {
  readonly #clock: () => Date;
  readonly #lifetimeMs: number;
  readonly #pending = new Map<string, PendingExperimentConfirmation<T>>();

  public constructor(
    clock: () => Date = () => new Date(),
    lifetimeMs = EXPERIMENT_LIMITS.confirmationTtlMs,
  ) {
    if (!Number.isSafeInteger(lifetimeMs) || lifetimeMs < 1_000 || lifetimeMs > 15 * 60_000) {
      throw new TypeError('Experiment confirmation lifetime is invalid.');
    }
    this.#clock = clock;
    this.#lifetimeMs = lifetimeMs;
  }

  public issue(payload: T, senderId: number, windowId: number): ExperimentConfirmationPreview<T> {
    this.prune();
    const token = randomBytes(32).toString('base64url');
    const previewHash = experimentSemanticHash(payload);
