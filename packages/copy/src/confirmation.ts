...
import { COPY_LIMITS } from './constants.js';
import { CopyError } from './errors.js';
import { copySemanticHash } from './identity.js';

export interface CopyConfirmationPreview<T> {
...
export class CopyConfirmationBroker<T> {
  readonly #now: () => number;
  ...
  public issue(payload: T, senderId: number, windowId: number): CopyConfirmationPreview<T> {
    const token = this.#tokenFactory();
    const expiresAt = new Date(this.#now() + COPY_LIMITS.confirmationTtlMs).toISOString();
    const previewHash = copySemanticHash(payload);
  ...
  public consume(token: string, previewHash: string, senderId: number, windowId: number): T {
    ...
    if (Date.parse(pending.expiresAt) <= this.#now()) {
      throw new CopyError('COPY_CONFIRMATION_EXPIRED');
    }
