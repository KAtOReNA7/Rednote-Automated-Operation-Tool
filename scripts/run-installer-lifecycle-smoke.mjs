  ...
  const { stdout } = await run(
    ...
    [
      ...
      [
        ...
        '$processes=@(Get-CimInstance -ClassName Win32_Process -ErrorAction SilentlyContinue | Where-Object { $_.ExecutablePath -and ($_.ExecutablePath.StartsWith($target,[System.StringComparison]::OrdinalIgnoreCase) -or $_.ExecutablePath.StartsWith($temp,[System.StringComparison]::OrdinalIgnoreCase)) } | ForEach-Object { $parent=Split-Path -Parent $_.ExecutablePath; [PSCustomObject]@{ image=[System.IO.Path]::GetFileName($_.ExecutablePath); inInstall=$_.ExecutablePath.StartsWith($target,[System.StringComparison]::OrdinalIgnoreCase); nsisHelper=$_.ExecutablePath.StartsWith($temp,[System.StringComparison]::OrdinalIgnoreCase) -and (Split-Path -Leaf $parent) -match "^ns.*\\.tmp$"; pid=[int]$_.ProcessId } })',
