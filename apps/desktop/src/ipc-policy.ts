...
function validArguments(operation: DesktopIpcOperation, args: readonly unknown[]): boolean {
  switch (operation) {
    ...
    case 'confirmSpoilerQuality': {
      ...
      return (
        value?.confirmation === 'SAVE_SPOILER_QUALITY_CHECK' &&
        Number.isSafeInteger(value.expectedRevision) &&
        Number(value.expectedRevision) >= 0 &&
        typeof value.previewHash === 'string' &&
        /^[a-f0-9]{64}$/u.test(value.previewHash) &&
    ...
    case 'confirmCopyIntegrity': {
      ...
      return (
        value?.confirmation === 'SAVE_COPY_INTEGRITY_CHECKS' &&
        Number.isSafeInteger(value.expectedRevision) &&
        Number(value.expectedRevision) >= 0 &&
        typeof value.previewHash === 'string' &&
        /^[a-f0-9]{64}$/u.test(value.previewHash) &&
