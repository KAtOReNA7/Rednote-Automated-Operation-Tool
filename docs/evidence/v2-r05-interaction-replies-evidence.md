...
- 第二次体验候选 `fb2c0fb6a7e05ffe058e776abe407b2fbc9e143e` 的 push / pull request Windows required CI 均成功；用户随后确认“不关联”可保存，但任意真实内容包关联均提示“互动请求不符合本地合同”。只读、去标识化核验现有体验数据得到 3 个内容包、1 个 workspace、0 个版本孤儿，且 3 个内容包 ID 均包含大写字母；未读取或输出标题、正文、评论、私信、真实 ID 或绝对路径。
- 根因为 R04 内容包权威 token 合同允许大小写 ASCII，而 R05 互动 token 校验意外只接受小写。修复仅使 R05 复用同一大小写语义并原样保留 ID；长度、字符集、exact-object、当前 workspace 存在性、revision 与数据库外键仍 fail closed，无 migration 或数据改写。
...
| R05-04 | 评论与私信均接受当前 workspace 的真实 R04 package ID（含大写）；不存在 ID 仍拒绝，异步恢复也不会提交原型 ID    | `tests/v2-interaction.test.tsx`                                           |
...
- `npm run format-check`、`npm run lint`、`npm run typecheck`、`npm run build` 与 `git diff --check` 通过；变更增量 secret/egress、skip/todo/only、V2 IPC 数和依赖文件检查均通过。两轮用户验收修复覆盖延迟持久化恢复，以及评论/私信关联含大写的真实 R04 ID；互动专项 9/9、最终聚焦集合 24/24 通过。
...
- 产品源码净新增 `1,400 LOC`（新增 1,539、删除 139）；测试净新增 `764 LOC`（新增 778、删除 14）。
