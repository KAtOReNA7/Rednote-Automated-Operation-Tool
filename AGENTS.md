...
- M0、M1、M2 与 M3 Issue 022—026 已完成。Issue 027 尚未开始；只有收到明确授权才可进入。
...
- 全量 Vitest 最多一次且仅在最终候选状态运行；专项脚本已选择相同文件后，不再通过多个脚本
  无差别重复这些文件。
...
- CI 可选择单次全量 Vitest或互不重叠的稳定分片；不得先串行运行所有重叠专项子集再运行全量。
